...
class TechSmithURLProvider(URLGetter):
    ...
    input_variables = {
        ...
        "major_release_version": {
            "required": True,
            "description": "Used to define which major release to download the latest version for"
        },
    }
    output_variables = {
    ...
    def main(self):
        ...
        major_release_version = self.env["major_release_version"]
        if major_release_version != "":
            # Get version information for specific version with the greatest VersionID
            length = len(major_release_version)
            two_digit_year = major_release_version[length - 2 :]
            data = sorted(data, key=lambda x: int(x["Major"]), reverse=True)
            version_list = []
            versionid_list = []
            for d in data:
                if d["Major"] == int(two_digit_year):
                    vers = "%s.%s.%s" % (d["Major"], d["Minor"], d["Maintenance"])
                    version_list.append(vers)
                    versionid = d["VersionID"]
                    versionid_list.append(versionid)
            vers = version_list[0]
            versionid = versionid_list[0]
        else:
            # Get version information for the latest version with the greatest VersionID
            data = sorted(data, key=lambda x: int(x["Major"]), reverse=True)
